...
def get_ai_system_prompt(user=None):
    """
    Menggabungkan SYSTEM_PROMPT_BASE dengan additional prompt dari konfigurasi
    dan konteks user jika tersedia.

    Args:
        user: (optional) User object untuk inject konteks user (role, permissions, dll)

    Returns:
        str: System prompt lengkap untuk AI
    """
    from apps.ai_assistant.models import AIAssistantConfig

    config = AIAssistantConfig.load()

    # Mulai dengan base prompt
    parts = [SYSTEM_PROMPT_BASE]

    # Tambahkan additional prompt dari DB config
    if config and config.system_prompt:
        parts.append(f"\nINSTRUKSI TAMBAHAN:\n{config.system_prompt}")

    # Inject konteks user jika tersedia
