        ...
        with transaction.atomic():
            for item in self.items.all():
                try:
                    ...
                    # Produk belum punya record stok di gudang ini → log dan raise
                    import logging
                    ...
                    # Stok sedang di-lock transaksi lain → log dan raise agar rollback
                    import logging
                    logging.getLogger(__name__).warning(
                        f"Stok terkunci untuk item {item} di gudang {self.gudang}"
                    )
                    raise
